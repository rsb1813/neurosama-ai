...
## Neru 직접 Codex OAuth 전환 (2026-07-23)

- [x] app-server 제거와 직접 OAuth 전송이라는 사용자 목표 확인.
- [x] 대안과 인증 경계 설계 승인.
- [x] 사양 문서 작성과 자체 검토.
- [x] 사용자 사양 문서 검토.
- [x] 상세 구현 계획 작성.
- [x] 직접 Device OAuth와 암호화 자격 증명 저장 구현.
- [x] 직접 Codex 스트리밍과 함수 도구 전송 구현.
- [x] app-server 의존성 제거와 집중 회귀 검증.
- [ ] 새 앱 데이터에서 Device OAuth 로그인과 실제 응답을 수동 검증.
- [x] Device OAuth 코드 발급 시 공식 로그인 페이지를 기본 브라우저로 열기.
- [x] 로그인 코드 발급 대기 중 즉시 진행 상태를 표시하기.
- [ ] 관련 집중 테스트와 ESLint를 통과한 뒤 앱에서 브라우저 열기를 검증하기.
- [x] 설정 renderer 재로딩 뒤 진행 중인 Device OAuth 코드를 복구하기.
- [x] 중복 로그인 요청을 새 요청이 아닌 기존 로그인 조회로 처리하기.

---

## Codex 원격 모델 새로고침 (2026-07-24)

- [x] 공식 Codex 원격 모델 조회 경로와 응답 계약 확인.
- [x] 직접 OAuth 구조를 유지하는 새로고침 설계 승인.
- [x] 설계 문서 작성과 자체 검토.
- [x] 상세 구현 계획 작성.
- [x] 구현 계획 사용자 승인.
- [x] 원격 모델 정상·오류 응답과 실패 후 마지막 정상 목록 보존 테스트 추가.
- [x] 최소 구현과 집중 테스트 12개 및 변경 파일 ESLint 통과.
- [~] stage-tamagotchi 타입 검사는 변경 파일 오류 없이 기존 `server-schema` 누락 1건과 상태 타입 불일치 2건에서 실패.
- [x] 화면 진입 자동 조회를 제거하고 버튼 클릭당 원격 요청 1회로 제한.
- [ ] 실제 로그인 계정에서 Terra 표시와 응답을 수동 검증.

---

## Codex turn 실패 진단 및 복구 (2026-07-24)

- [x] 일반 오류 문구가 원인을 숨기는 실패를 재현하고 원본 오류 전달 테스트를 추가.
- [x] 실제 Electron 로그에서 `Unsupported service_tier: auto` 응답을 확인.
- [x] `auto` 선택 시 서비스 티어를 요청에서 생략하도록 수정하고 회귀 테스트를 추가.
- [x] Codex 관련 테스트 25개, 변경 파일 ESLint, `git diff --check` 통과.
- [x] 재시작한 Electron 앱에서 실제 Codex 응답 수신 확인.

---

## 자동 발화 1회 및 쓰레기통 새 세션 (2026-07-24)
