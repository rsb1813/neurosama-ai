...
2. **Barge-in (M-G) — IN DESIGN** on `feat/neru-barge-in` (brainstorming). Readiness map from codebase exploration (2026-07-15):
   ...
   - **Design + spec + plan done; ready to implement.** Spec: `docs/superpowers/specs/2026-07-15-neru-barge-in-design.md` (`35b48de`). Plan: `docs/superpowers/plans/2026-07-15-neru-barge-in.md` (`0604964`). Approach A+C: Silero VAD speech-start trigger + dedicated `useBargeIn` composable; build the LLM `AbortController` + `abortActiveStream` (abortSignal already plumbed to `streamText`); reuse the existing TTS stop path with a new `'barge-in'` stop reason; keep the partial reply on abort (D3). Plan = 4 TDD tasks: (1) in-flight abort, (2) persist partial on abort (`performSend` catch handles abort gracefully — verified from source that abort currently hits the failure path), (3) `useBargeIn` composable, (4) Stage wiring. Awaiting user choice of execution mode (subagent-driven vs inline).
