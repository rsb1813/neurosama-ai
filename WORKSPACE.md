...
2. **Barge-in (M-G) — IN DESIGN** on `feat/neru-barge-in` (brainstorming). Readiness map from codebase exploration (2026-07-15):
   ...
   - **Design approved; spec written** → `docs/superpowers/specs/2026-07-15-neru-barge-in-design.md` (commit `35b48de`). Approach A+C: Silero VAD speech-start trigger + dedicated `useBargeIn` composable; build the LLM `AbortController` (abortSignal already plumbed to `streamText`); reuse the existing TTS stop path with a new `'barge-in'` stop reason. Decision: keep the partial spoken reply in history (D3 — persistence on abort flagged for the plan to verify). Awaiting user spec review, then writing-plans.
