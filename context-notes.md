...
## Neru 제품 랜딩 페이지 결정 (2026-07-19)

- 대상은 일반 방문자, 개발자, 오픈소스 기여자이며 캐릭터 소개에서 실제 시스템과 기여 안내로 이어지는 단일 페이지를 만든다.
- 영어를 기본값으로 하고 중국어 간체, 일본어, 한국어의 완전한 번역 세트를 함께 제공한다. 선택 언어만 브라우저에 저장하며 외부 번역 API는 사용하지 않는다.
- 현재 개발 단계에는 다운로드나 공개 데모보다 GitHub 저장소를 주 CTA로, 작동 방식 섹션 이동을 보조 CTA로 두는 것이 정직하다.
- 시각 방향은 사용자가 `Arcane Editorial`을 선택했다. 나머지 방향은 AI 생성물처럼 느껴진다는 피드백에 따라 과도한 네온, 그라데이션, 유리 효과, 반복 카드 표현을 제외한다.
- 히어로는 실제 `neru-render-airi.png`의 마녀 Live2D 렌더를 중앙에 크게 배치하는 `Character Centerpiece` 구성을 선택했다. 생성형 캐릭터 이미지는 사용하지 않는다.
- 사이트는 기존 AIRI 런타임과 분리된 루트 `site/`에 두고 정적 단일 페이지로 만든다. 인증, 데이터베이스, 분석, 업로드, 라이브 챗은 범위 밖이다.
- 최종 결과는 로컬 비공개 미리보기로만 저장한다. 사용자의 명시적 요구에 따라 Sites 호스팅과 공개 URL 생성은 수행하지 않는다.
- 승인된 전체 설계는 `docs/superpowers/specs/2026-07-19-neru-landing-page-design.md`에 기록한다.
- 구현 계획은 `docs/superpowers/plans/2026-07-19-neru-landing-page.md`에 기록한다. Sites vinext 스타터를 `site/`에 격리하고, Node 기본 테스트 러너로 언어 폴백과 서버 렌더를 검증한 뒤 로컬 개발 서버의 비공개 URL만 전달하는 순서다.
- Task 1에서 Sites vinext 스타터를 `site/`에 초기화하고 인증·DB 예제와 일회성 미리보기 의존성을 제거했다. 테스트 3개와 빌드가 통과했으며 `http://localhost:3000/`이 HTTP 200으로 앱 브라우저에 표시되는 것까지 확인했다.
- Task 1 검토에서 기존 README의 인증·D1·Drizzle 안내와 빌드 플러그인의 선택적 drizzle 복사 검사가 경미한 잔여 항목으로 기록됐다. 기능 구현을 방해하지 않으므로 최종 정리 단계에서 최소 범위로 판단한다.
- Task 2에서 영어 기본값과 중국어 간체·일본어·한국어 전체 문구를 단일 언어 모듈로 추가했다. 선택 언어는 `neru-locale`로 저장하고, 저장소 읽기뿐 아니라 `window.localStorage` 접근자 자체가 예외를 던지는 환경도 영어로 폴백하도록 회귀 테스트로 보호한다.
- Task 3에서 실제 `neru-render-airi.png`를 중앙 캐릭터 스터디로 사용한 Arcane Editorial 전체 페이지를 구현했다. 모바일·키보드 포커스·감소 모션을 포함하며, 화면 문구뿐 아니라 홈·주 탐색·이미지 대체 텍스트도 네 언어로 함께 전환한다.

## Neru 랜딩 페이지 최종 비공개 미리보기 검증 (2026-07-19)

- Task 4에서 승인된 편집 매거진 스타일 소셜 카드를 중앙 기준으로 1200×630 PNG로 변환해 `site/public/og.png`에 저장했다. 원본은 그대로 유지했고, 카드에는 승인된 제목·마스트헤드·푸터만 있으며 캐릭터 아트나 금지된 시각 효과를 추가하지 않았다.
- `site/app/layout.tsx`는 요청의 `host`와 `x-forwarded-proto`를 사용해 절대 Open Graph 및 X 이미지 URL을 만든다. 로컬 렌더링은 `http://localhost/og.png`를 검증했으며, 배포 호스트에서는 HTTPS를 기본값으로 사용한다.
- 소셜 메타데이터 회귀 테스트를 먼저 추가해 정적 메타데이터 상태에서 RED를 확인한 뒤 구현했다. 최종 `npm test`는 렌더링 테스트 4개 모두 통과했고, `npm run lint`도 오류 없이 통과했다.
- 이번 작업에서는 Sites 호스팅, 배포 명령, 호스팅 미리보기 API를 호출하지 않았다. 최종 산출물은 로컬 비공개 미리보기 범위에만 유지한다.
