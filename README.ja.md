...
全体像と各フェーズの状態: **[`ROADMAP.md`](ROADMAP.md)** — 9 つのサブプロジェクト
（音声コア、長期記憶、能動発話、チャット統合、配信、ゲームエージェント、
コンピュータ制御 / コーディングエージェント、マルチペルソナ「Evil neru」、
YouTube 同時視聴）。MVP（音声コア）の概要:

- ☑ AIRI フォークを唯一のシステムとして起動、プロキシ経由でローカル LLM に接続
- ☑ `neru-audio` GPU ゲートウェイ（Chatterbox TTS + faster-whisper STT）の自動起動
- ☑ 完全なライブループ（マイク → STT → LLM → TTS → アバター）検証済み
- ☑ neru ペルソナ / キャラクターカード（プリシード・アクティブ）
- ◐ 二言語出力 — **英語音声 + 韓国語字幕**: 音声・チャットパネルの韓国語は動作、字幕オーバーレイは進行中
- ☐ 割り込み（話すと neru が即停止）
