...
첫 실행 선택 화면은 추가하지 않는다. 새 설치에서는 LLM 미선택 상태를 유지하고, 기존 설치의 제공자 설정은 보존한다. STT와 TTS는 현재 로컬 `neru-audio` 구성을 계속 사용한다.
...
- 새 설치는 `settings/consciousness/active-provider`를 비워 둔다.
- 기존 설치의 active provider와 자격 증명은 마이그레이션 과정에서 변경하지 않는다.
